...
export default function SermonFilters({
  ...
  return (
    <div className={`flex gap-2 sm:gap-3 items-start ${open ? "mb-2 sm:mb-3" : ""}`}>
      <div className="shrink-0 flex flex-col items-start gap-1">
        {toggleButton}
        {open && hasActiveFilters && (
          <button
            onClick={handleClear}
            className="inline-flex items-center gap-1.5 px-3 py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 cursor-pointer transition-colors rounded-md hover:bg-gray-200 dark:hover:bg-gray-800"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
      ...
      <div className={`flex-1 min-w-0 flex-wrap items-center gap-2 sm:gap-3 justify-end ${open ? "flex" : "hidden"}`}>
        <Listbox value={preacher} onChange={onPreacherChange}>
          ...
          <ListboxOptions anchor="bottom end" transition className={`${panelClass} w-64 transition duration-100 ease-out data-closed:opacity-0 data-closed:scale-95`}>
        ...
        <Listbox value={series} onChange={onSeriesChange}>
          ...
          <ListboxOptions anchor="bottom end" transition className={`${panelClass} w-72 transition duration-100 ease-out data-closed:opacity-0 data-closed:scale-95`}>
            <ListboxOption value="" className={optionClass}>
              All series
        ...
        <Combobox value={keyword} onChange={(v) => { onKeywordChange(v ?? ""); setKeywordQuery(""); }} onClose={() => setKeywordQuery("")}>
          ...
          <ComboboxOptions anchor="bottom end" transition className={`${panelClass} w-56 transition duration-100 ease-out data-closed:opacity-0 data-closed:scale-95`}>
